...
function Dashboard() {
  ...
  const dashboardStats = useMemo(() => {
    ...
    let failed = 0;
    let booked = 0;
    const liveCalls = [] as typeof calls;
    const cells: number[] = new Array(60).fill(0);
    const inFlightByAgent = new Map<string, number>();
    const doneTodayByAgent = new Map<string, number>();
    const campaignStats = new Map<string, { total: number; booked: number }>();

    for (const c of calls) {
      const t = new Date(c.started_at).getTime();
      const byCampaign = c.campaign_id ? campaignStats.get(c.campaign_id) ?? { total: 0, booked: 0 } : null;
      if (byCampaign && c.campaign_id) {
        byCampaign.total += 1;
        if (c.appointment_booked) byCampaign.booked += 1;
        campaignStats.set(c.campaign_id, byCampaign);
      }
      if (c.status === "in_progress") {
        liveCalls.push(c);
        if (c.agent_id) inFlightByAgent.set(c.agent_id, (inFlightByAgent.get(c.agent_id) ?? 0) + 1);
      }
      if (t >= startToday) {
        callsToday += 1;
        if (["completed", "voicemail", "in_progress"].includes(c.status)) answered += 1;
        if (c.status === "completed") {
          completed += 1;
          if (c.agent_id) doneTodayByAgent.set(c.agent_id, (doneTodayByAgent.get(c.agent_id) ?? 0) + 1);
          if (c.appointment_booked) booked += 1;
        }
        if (c.status === "voicemail") voicemails += 1;
  ...
  const roster = useMemo(
    () =>
      agents.map((a) => {
        const inFlight = dashboardStats.inFlightByAgent.get(a.id) ?? 0;
        const doneToday = dashboardStats.doneTodayByAgent.get(a.id) ?? 0;
        return { agent: a, inFlight, doneToday };
      }),
    [agents, dashboardStats],
  );
    ...
    <>
      ...
      <div className="mb-6 rounded-xl border border-black/5 bg-white shadow-sm overflow-hidden">
        <div className="grid grid-cols-1 md:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)] divide-y md:divide-y-0 md:divide-x divide-neutral-200/70">
          <div className="p-5">
            ...
            <div className="mt-2 flex items-end gap-3">
              <span className="text-5xl font-mono font-medium tracking-tight text-neutral-900 tabular-nums">
                {String(dashboardStats.liveCalls.length).padStart(2, "0")}
              </span>
              <span className="pb-2 text-xs text-neutral-500">
                {dashboardStats.liveCalls.length === 1 ? "line active" : "lines active"}
              </span>
            </div>
          ...
          <SitrepCell icon={PhoneCall} label="Placed today" value={dashboardStats.callsToday} />
          <SitrepCell
            icon={CalendarCheck}
            label="Booked"
            value={dashboardStats.booked}
            sub={dashboardStats.completed ? `${Math.round((dashboardStats.booked / dashboardStats.completed) * 100)}% of completed` : "no completed calls"}
          />
          <SitrepCell
            icon={Voicemail}
            label="Voicemail / Failed"
            value={`${dashboardStats.voicemails} / ${dashboardStats.failed}`}
            sub={dashboardStats.callsToday ? `${Math.round((dashboardStats.answered / dashboardStats.callsToday) * 100)}% answered` : "-"}
          />
        </div>
        ...
        <div className="border-t border-neutral-200/70 px-5 pt-4 pb-5">
          <div className="flex items-center justify-between text-[10px] font-mono uppercase tracking-widest text-neutral-500 mb-2">
            ...
            <span>peak {dashboardStats.heatMax}/min</span>
          </div>
          <div className="flex items-end gap-[2px] h-14">
            {dashboardStats.heat.map((v, i) => {
              const h = Math.max(2, Math.round((v / dashboardStats.heatMax) * 100));
              return (
                <div
                  key={i}
                  className={cn(
                    "flex-1 rounded-t-sm",
                    v === 0 ? "bg-neutral-100" : v >= dashboardStats.heatMax * 0.66 ? "bg-brand-primary" : v >= dashboardStats.heatMax * 0.33 ? "bg-brand-primary/60" : "bg-brand-primary/30",
                  )}
                  style={{ height: `${h}%` }}
        ...
        <div className="lg:col-span-3 rounded-xl border border-black/5 bg-white shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-neutral-200/70 flex items-center justify-between">
            ...
            <span className="text-[10px] font-mono text-neutral-500">
              {dashboardStats.liveCalls.length} open
            </span>
          </div>
          <div className="divide-y divide-neutral-100 max-h-[360px] overflow-y-auto">
            {dashboardStats.liveCalls.length === 0 && (
            ...
            {dashboardStats.liveCalls.map((c) => {
      ...
      <div className="rounded-xl border border-black/5 bg-white shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-neutral-100">
          {campaigns.slice(0, 6).map((c) => {
             const stats = dashboardStats.campaignStats.get(c.id) ?? { total: 0, booked: 0 };
             const cConv = stats.total ? (stats.booked / stats.total) * 100 : 0;
            const agent = agents.find((a) => a.id === c.agent_id);
            return (
              <div key={c.id} className="px-5 py-4 grid grid-cols-1 md:grid-cols-[1.5fr_auto_1fr_1fr] items-center gap-3">
                ...
                <div className="min-w-[120px]">
                  <div className="flex items-center justify-between text-[10px] font-mono text-neutral-500 mb-1">
                    <span>calls</span>
                    <span className="text-neutral-900 tabular-nums">{stats.total}</span>
                  </div>
                  <div className="h-1 rounded-full bg-neutral-100 overflow-hidden">
                    <div
                      className="h-full bg-neutral-800"
                      style={{ width: `${Math.min(100, stats.total ? Math.log(stats.total + 1) * 22 : 0)}%` }}
                    />
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-mono text-brand-primary tabular-nums">
                    {stats.total ? `${cConv.toFixed(1)}%` : "-"}
                  </p>
